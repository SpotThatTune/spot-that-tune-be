...
io.sockets.on('connection', socket => {
  ...
  socket.on('JOIN', ({ gameId, user }) => {
    ...
    if(games[game]){
      socket.join(game);
      const currentPlayers = games[game].players;
      currentPlayers.push({ id:socket.id, user, score:0 });
      console.log(games[game]);
      io.sockets.in(game).emit('GAME_INFO', { gameId:game, game:games[game] });
    } else {
      io.sockets.to(socket.id).emit('JOIN_FAIL');
      console.log(`fail on ${socket.id}`);
    }
  });
